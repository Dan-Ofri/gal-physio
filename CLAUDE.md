...
- Because no text is orange any more, the light sections carry their brand colour through `.heading-rule` (a tangerine bar under a section heading) and a handful of fills. Strip those and the page renders as graphite on cream with no palette at all — that is what happens if you only swap tokens and skip the placement.

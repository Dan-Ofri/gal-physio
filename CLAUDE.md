...
- **`stroke-width` is normalised by rendered pixels, not by the attribute.** It lives in the 24-unit viewBox, so thickness is `stroke-width x px/24`. The standard is **~1.5px rendered**, which means the attribute scales with the box: 2 at 20px, 2.5 at 16px and 14px, 3 at 12px, 1.5 at 32px. Flattening every icon to `stroke-width="2"` looks like consistency and produces the opposite — it would make the Hero secondary chevron thinner than the calendar beside it in the same flex row, and turn the 12px cancellation marks into 1px hairlines. The three exceptions carry comments saying so.
